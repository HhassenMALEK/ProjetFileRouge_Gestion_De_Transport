...
public interface CarPoolingRepository extends JpaRepository<CarPooling, Long>{
   ...
   //List des covoiturages aprés une date
    ...
    @Query("SELECT c FROM CarPooling c WHERE c.organizer.id = :userId AND c.departureDate = :date AND c.departureTime = :time AND (:excludeId IS NULL OR c.id <> :excludeId)")
    ...
    @Query("SELECT c FROM CarPooling c WHERE c.vehicle.id = :vehicleId AND c.departureDate = :date AND c.departureTime = :time AND (:excludeId IS NULL OR c.id <> :excludeId)")
    ...
    // Liste par date
    List<CarPooling> findByDepartureDate(LocalDate date);
